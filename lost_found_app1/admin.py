...
class ClaimAdmin(admin.ModelAdmin):
    list_display = ('id', 'user', 'foundItem', 'status', 'created_at', 'updated_at', 'display_supporting_images')
    list_filter = ('status', 'created_at', 'updated_at')
    search_fields = ('foundItem__title', 'claimDescription', 'user__username', 'user__email')
    readonly_fields = ('created_at', 'updated_at', 'display_supporting_images')
    ...
    fieldsets = (
        ...
        ('Supporting Evidence', {
            'fields': ('supportingImages', 'display_supporting_images')
        }),
        ('Status & Review', {
    ...
    def display_supporting_images(self, obj):
        """Show supporting images as thumbnails in admin list/detail view"""
        if obj.supportingImages:
            from django.utils.html import format_html
            images = obj.get_supporting_images_list()
            thumbnails = [f'<img src="{img}" style="height:50px;margin-right:5px;" />' for img in images]
            return format_html(''.join(thumbnails))
        return "-"
    display_supporting_images.short_description = 'Supporting Images'
########################################################################################################################################
