...
class ClaimSerializer(serializers.ModelSerializer):
    ...
    supportingImages = serializers.ListField(child=serializers.CharField(), read_only=True)

    # -------------------
    # Input fields (mobile camelCase)
    # -------------------
    ...
    claimDescription = serializers.CharField(source='claim_description', required=True)
    proofOfOwnership = serializers.CharField(source='proof_of_ownership', required=False, allow_blank=True)
    adminNotes = serializers.CharField(source='admin_notes', required=False, allow_blank=True, allow_null=True)
    supportingImagesInput = serializers.CharField(write_only=True, required=False, allow_blank=True, allow_null=True)
    ...
    # -------------------
    # Validation + duplicate claim check
    # -------------------
    def validate(self, attrs):

        # Convert images
        imgs = attrs.pop('supportingImagesInput', None)
        if imgs:
            if isinstance(imgs, str):
                imgs_list = imgs.replace("[", "").replace("]", "").replace("'", "").split(",")
                attrs['supporting_images'] = [i.strip() for i in imgs_list if i.strip()]
        ...
        user = request.user
        found_item = attrs.get("found_item")
