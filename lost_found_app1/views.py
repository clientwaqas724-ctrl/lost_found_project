...
class ClaimViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        user = self.request.user
        found_item = self.request.data.get("foundItem")

        # Prevent duplicate claim (safety before DB insert)
        if Claim.objects.filter(user=user, found_item_id=found_item).exists():
            raise ValidationError({"detail": "You have already submitted a claim for this item."})

        try:
            serializer.save(user=user)
        except IntegrityError:
            # If DB unique constraint still triggers
            raise ValidationError({"detail": "You have already submitted a claim for this item."})
