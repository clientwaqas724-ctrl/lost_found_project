...
class Claim(models.Model):
    ...
    user = models.ForeignKey(
        ...
        related_name='claims',
        null=True,  # Make nullable temporarily for debugging
        blank=True
    )
    foundItem = models.ForeignKey(
        FoundItem,
        on_delete=models.CASCADE,
        related_name='claims'
    )
    ...
    # Accept comma-separated string of image URLs
    ...
    def __str__(self):
        ...
        return f"Claim {self.id} for {self.foundItem.title} by {user_name}"
    
    def get_supporting_images_list(self):
        """Convert comma-separated string to list of URLs"""
        if self.supportingImages:
            return [img.strip() for img in self.supportingImages.split(',') if img.strip()]
        return []
#######################################################################################################################
#######################################################################################################################
#######################################################################################################################
